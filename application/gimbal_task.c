...
#elif PITCH_CURRENT_ROUTE == PITCH_CURRENT_ROUTE_20A
  can2_yaw_current = pitch_current;
#else
#error "Invalid PITCH_CURRENT_ROUTE value"
#endif

#if SHOOT_CURRENT_ROUTE == SHOOT_CURRENT_ROUTE_207
  can1_shoot_current = shoot_current;
#elif SHOOT_CURRENT_ROUTE == SHOOT_CURRENT_ROUTE_208
  can1_rev_current = shoot_current;
#else
...
void gimbal_task(void const *pvParameters)
{
    ...
    while (1)
    {
        ...
        if (!(toe_is_error(YAW_GIMBAL_MOTOR_TOE) && toe_is_error(PITCH_GIMBAL_MOTOR_TOE) && toe_is_error(TRIGGER_MOTOR_TOE)))
        {
            ...
            {
    				gimbal_send_actuator_currents(0, 0, 0);
				
            }
            else
            {
    				gimbal_send_actuator_currents(yaw_can_set_current, pitch_can_set_current, shoot_can_set_current);
